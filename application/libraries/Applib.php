...
function curl_get_contents($url)
{
  ...
  static function pc(){
    ...
    function get_updates($remote = FALSE) {
        ...
        if ($remote) {
            ...
            curl_setopt_array($ch, array(
                CURLOPT_URL => UPDATE_URL."check.php?version=".$version."&build=".$build,
        ...
        return $this->ci->db->where('build >',$build)->where('installed','0')->get('updates')->result();
    }
    ...
    function get_xrates() {
        ...
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => "http://jsonrates.com/get/?base=".$currency."&apiKey=jr-f2c427c3ce57e81ad2d2218370950e40",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_FOLLOWLOCATION => true
        ));
        $output = curl_exec($ch);
        ...
        $this->ci->db->where('code',$currency)->update('currencies',array('xrate' => '1'));
    ...
    public function create_pdf($pdf)
    {
            ...
            $mpdf=new mPDF('','A4','','',15,15,24,16,9,9,'P');
    ...
    function get_activities() {
        ...
        return $this->ci->db->where('activity_date >',date("Y-m-d H:i:s",$lastseen))->get('activities')->result();
    }
